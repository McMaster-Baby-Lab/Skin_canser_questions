...
expName = 'FaceAttractiveness2'  # from the Builder filename that created this script
expInfo = {
    'Name': '',
    'Gender': '',
    'Age': '',
...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='C:\\Users\\annak\\Documents\\GitHub\\faceattractiveness_lesion\\FaceAttractiveness2_lastrun.py',
...
face_image = visual.ImageStim(
    win=win,
    name='face_image', units='pix', 
    image='sin', mask=None, anchor='center',
    ori=0, pos=(0, 280), size=None,
    color=[1,1,1], colorSpace='rgb', opacity=1,
    flipHoriz=True, flipVert=False,
    texRes=512, interpolate=False, depth=-1.0)
slider_treatment_surgery = visual.Slider(win=win, name='slider_treatment_surgery',
    startValue=None, size=(1.0, 0.025), pos=(0, 0), units=None,
    labels=("0% Never", "100% Always"), ticks=(0,25,50,75,100), granularity=0,
    style=['rating'], styleTweaks=('labels45', 'triangleMarker'), opacity=1,
...
slider_treatment_radiation = visual.Slider(win=win, name='slider_treatment_radiation',
    startValue=None, size=(1.0, 0.025), pos=(0, -0.3), units=None,
    labels=("0% Never", "100% Always"), ticks=(0,25,50,75,100), granularity=0,
    style=['rating'], styleTweaks=('triangleMarker',), opacity=1,
    labelColor='Black', markerColor='Red', lineColor='Black', colorSpace='rgb',
    font='HelveticaBold', labelHeight=0.03,
    flip=False, ori=0, depth=-5, readOnly=False)
text_treatment_surgery = visual.TextStim(win=win, name='text_treatment_surgery',
    text='How likely are you to recommend surgery? ',
    font='Open Sans',
    pos=(0, 0.05), height=0.03, wrapWidth=None, ori=0.0, 
    color='black', colorSpace='rgb', opacity=None, 
    languageStyle='LTR',
    depth=-6.0);
text_treatment_radiation = visual.TextStim(win=win, name='text_treatment_radiation',
    text='How likely are you to recommend radiation? ',
    font='Open Sans',
...
for thisOriginalFaceTrial in originalFaceTrials:
    ...
    face_image.setImage(faceImages)
    slider_treatment_surgery.reset()
    # setup some python lists for storing info about the mouse_5
    mouse_5.clicked_name = []
    gotValidClick = False  # until a click is received
    slider_treatment_radiation.reset()
    # keep track of which components have finished
    RatingTrialComponents = [face_image, slider_treatment_surgery, NextBu, mouse_5, slider_treatment_radiation, text_treatment_surgery, text_treatment_radiation]
    ...
    while continueRoutine:
        ...
        if (slider_treatment_surgery.getRating()) and (slider_treatment_radiation.getRating()):
        ...
        # *slider_treatment_surgery* updates
        if slider_treatment_surgery.status == NOT_STARTED and tThisFlip >= 0.3-frameTolerance:
            # keep track of start time/frame for later
            slider_treatment_surgery.frameNStart = frameN  # exact frame index
            slider_treatment_surgery.tStart = t  # local t and not account for scr refresh
            slider_treatment_surgery.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(slider_treatment_surgery, 'tStartRefresh')  # time at next scr refresh
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'slider_treatment_surgery.started')
            slider_treatment_surgery.setAutoDraw(True)
        ...
        if NextBu.status == NOT_STARTED and tThisFlip >= 0.3-frameTolerance:
            ...
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'NextBu.started')
        ...
        # *slider_treatment_radiation* updates
        if slider_treatment_radiation.status == NOT_STARTED and tThisFlip >= 0.3-frameTolerance:
            # keep track of start time/frame for later
            slider_treatment_radiation.frameNStart = frameN  # exact frame index
            slider_treatment_radiation.tStart = t  # local t and not account for scr refresh
            slider_treatment_radiation.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(slider_treatment_radiation, 'tStartRefresh')  # time at next scr refresh
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'slider_treatment_radiation.started')
            slider_treatment_radiation.setAutoDraw(True)
        
        # *text_treatment_surgery* updates
        if text_treatment_surgery.status == NOT_STARTED and tThisFlip >= 0.3-frameTolerance:
            # keep track of start time/frame for later
            text_treatment_surgery.frameNStart = frameN  # exact frame index
            text_treatment_surgery.tStart = t  # local t and not account for scr refresh
            text_treatment_surgery.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(text_treatment_surgery, 'tStartRefresh')  # time at next scr refresh
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'text_treatment_surgery.started')
            text_treatment_surgery.setAutoDraw(True)
        
        # *text_treatment_radiation* updates
        if text_treatment_radiation.status == NOT_STARTED and tThisFlip >= 0.3-frameTolerance:
            # keep track of start time/frame for later
            text_treatment_radiation.frameNStart = frameN  # exact frame index
            text_treatment_radiation.tStart = t  # local t and not account for scr refresh
            text_treatment_radiation.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(text_treatment_radiation, 'tStartRefresh')  # time at next scr refresh
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'text_treatment_radiation.started')
            text_treatment_radiation.setAutoDraw(True)
    ...
    for thisComponent in RatingTrialComponents:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    originalFaceTrials.addData('slider_treatment_surgery.response', slider_treatment_surgery.getRating())
    originalFaceTrials.addData('slider_treatment_surgery.rt', slider_treatment_surgery.getRT())
    # store data for originalFaceTrials (TrialHandler)
    x, y = mouse_5.getPos()
    buttons = mouse_5.getPressed()
    if sum(buttons):
        # check if the mouse was inside our 'clickable' objects
        ...
        for obj in clickableList:
            if obj.contains(mouse_5):
                gotValidClick = True
                mouse_5.clicked_name.append(obj.name)
    originalFaceTrials.addData('mouse_5.x', x)
    originalFaceTrials.addData('mouse_5.y', y)
    originalFaceTrials.addData('mouse_5.leftButton', buttons[0])
    originalFaceTrials.addData('mouse_5.midButton', buttons[1])
    originalFaceTrials.addData('mouse_5.rightButton', buttons[2])
    if len(mouse_5.clicked_name):
        originalFaceTrials.addData('mouse_5.clicked_name', mouse_5.clicked_name[0])
    originalFaceTrials.addData('slider_treatment_radiation.response', slider_treatment_radiation.getRating())
    originalFaceTrials.addData('slider_treatment_radiation.rt', slider_treatment_radiation.getRT())
